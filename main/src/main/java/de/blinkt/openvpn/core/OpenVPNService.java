        ...
        for (IpAddress ipIncl : routes.getNetworks(true))
        {
            try {
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) { // API 33
                    ...
                    // Для старых версий: вручную разбираем Prefix на адрес+маску
                    // Здесь предполагается, что ipIncl.getIPv4Address() и .networkMask доступны
                    builder.addRoute(ipIncl.getIPv4Address(), ipIncl.networkMask);
                }
        ...
        for (IpAddress ipExcl : routes.getNetworks(false))
        {
            try {
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) { // API 33
                    ...
                    // excludeRoute не поддерживается ниже API 33 — можно проигнорировать или логировать
                    VpnStatus.logWarning("excludeRoute not supported below API 33: " + ipExcl.getPrefix());
                }
            } catch (UnknownHostException | IllegalArgumentException ia) {
