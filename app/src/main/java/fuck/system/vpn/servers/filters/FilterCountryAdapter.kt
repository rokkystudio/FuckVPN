...
class FilterCountryAdapter(
    private val countries: List<FilterCountryItem>,
    private val onItemClicked: (position: Int) -> Unit
) : RecyclerView.Adapter<FilterCountryAdapter.CountryViewHolder>()
{
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CountryViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_filter_country, parent, false)
        return CountryViewHolder(view)
    }

    override fun onBindViewHolder(holder: CountryViewHolder, position: Int) {
        val item = countries[position]
        val code = item.country.lowercase()

        holder.countryFilterName.text = ServerGeo.getCountry(code)
        holder.countryFilterCheck.isChecked = item.enabled
        holder.countryFilterFlag.setImageResource(ServerGeo.getFlag(code))
